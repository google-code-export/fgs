FGS.ravenskyecity.MessageCenter = 
{
	Start: function(params, retry) {
		...
		$.ajax({
			type: "GET",
			url: 'https://apps.facebook.com/ravenskyecity/',
			dataType: 'text',
			success: function(dataStr)
			{
				...
				try
				{
					...
					FGS.ravenskyecity.MessageCenter.Click2(params);
				}
				catch(err)
				{
					FGS.dump(err);
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				try
				{
					...
					FGS.ravenskyecity.MessageCenter.Click3(params);
				}
				catch(err)
				{
					FGS.dump(err);
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					FGS.ravenskyecity.MessageCenter.Receive2(params);
				}
				catch(err)
				{
					FGS.dump(err);
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				try
				{
					...
					FGS.ravenskyecity.MessageCenter.Receive3(params);
				}
				catch(err)
				{
					FGS.dump(err);
...
FGS.ravenskyecity.Freegifts = 
{
	Click: function(params, retry)
	{
		...
		$.ajax({
			type: "GET",
			url: 'https://apps.facebook.com/ravenskyecity/'+addAntiBot,
			dataType: 'text',
			success: function(dataStr)
			{
				var dataStr = FGS.processPageletOnFacebook(dataStr);
				...
				try
				{
					...
					FGS.ravenskyecity.Freegifts.Click2(params);
				}
				catch(err)
				{
					FGS.dump(err);
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				try
				{
					...
					FGS.ravenskyecity.Freegifts.Click3(params);
				}
				catch(err)
				{
					FGS.dump(err);
				...
				try
				{
					...
					if(typeof(params.sendTo) == 'undefined')
					{
						if(frArr.length == 0)
						{
							...
							params.channel = 'https://fbcanvas.ravenskyecity.com/';
					...
					params.step4params = 
					{
						props: JSON.stringify({
							message: "Here's __GIFTNAME__ for your City in Ravenskye City! Could you help me by sending a gift back?".replace('__GIFTNAME__', giftName),
							data: {
								'requestType': "gift",
								'clickType': "20300",
								'item': {'ident': params.gift}
							},
							method: 'apprequests',
							to: params.sendTo
						})
					...
					*/
					...
					var reqData = {};
					if(friends.length > 0)
						reqData.filters = JSON.stringify( [{name: 'Ravenskye Friends', user_ids: friends}] );
					...
					FGS.ravenskyecity.Freegifts.Click4(params);
				}
				catch(err)
				{
					FGS.dump(err);
