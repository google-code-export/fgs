			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					if(el.length > 0)
					{
						...
						info.image = el.children('p:first').children('img').attr('path');
						info.title = gift;
						info.text  = from;
						info.time = Math.round(new Date().getTime() / 1000);
					...
					if($('#app101539264719_gift_items', dataHTML).length > 0)
					{
						...
						info.image = $('#app101539264719_gift_items', dataHTML).find('img:first').attr("path");
						info.title = gift;
						info.text  = from;
						info.time = Math.round(new Date().getTime() / 1000);
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					if( $(".reward-image-container",dataHTML).length > 0)
					{
						...
						{
							info.image = $('.reward-image-container-left', dataHTML).find('img:last').attr('path');
						}
						else
						{
							info.image = $(".reward-image-container",dataHTML).children('img').attr("path");
						}
					}
					else if($(".page-title",dataHTML).find('img').length > 0)
					{
						info.image = $(".page-title",dataHTML).find('img').attr("path");
					}
					else if($("#app101539264719_item_wrapper",dataHTML).find('.left_cell').find('img').length > 0)
					{
						info.image = $("#app101539264719_item_wrapper",dataHTML).find('.left_cell').find('img').attr("path");
					}
					else
					{
						info.image = $('div.cell_wrapper', dataHTML).find('img:first').attr('path');
					}					
					...
					else
					{
						...
						info.image = $("#app101539264719_item_wrapper",dataHTML).children('.pad:nth-child(3)').children('img').attr('path');
					}
