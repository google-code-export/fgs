	...
	Click2: function(params, retry)
	{
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				try
				{
					params.step3params = 'cachebuster='+Math.random()+'&filter=existing&ask_gift=-1&item_id='+params.gift+'&recipient_id=&thankyou_gift=0&default_tab=game&request_ids=&gift_icon=&'+params.step2params;
					params.step3url = 'http://fbcanvas.ravenwoodfair.com/app/1/gift/send';
		...
		$.ajax({
			...
			success: function(dataStr)
			{
				...
				try
				{
					...
					{
						var dataStr = tst[1];
						dataStr = dataStr.replace(/fb:r/gi, 'fb_r');
						var dataHTML = FGS.HTMLParser(dataStr);	
					}
					...
					if(el.length > 0)
					{
						...
						info.title = el.text().replace('You just accepted this', '').replace('from ','');
						info.text  = '';
						info.time = Math.round(new Date().getTime() / 1000);
						
						
						if($('input[name="item_id"]', dataHTML).length > 0)
						{
							info.thanks = 
							{
								gift: $('input[name="item_id"]', dataHTML).val()
							}
						}
